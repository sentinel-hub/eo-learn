...
def test_workflow_copying_eopatches():
    ...
    timestamps = ["2016-02-12", "2016-02-17"]
    ...
    results = workflow.execute({create_node: {"bbox": bbox, "timestamps": timestamps}})

    eop1 = results.outputs["out1"]
    eop2 = results.outputs["out2"]

    assert eop1 == EOPatch(bbox=bbox, timestamps=timestamps, data={"data2": np.ones((2, 4, 4, 3), dtype=np.uint8)})
    assert eop2 == EOPatch(bbox=bbox, timestamps=timestamps, data={"data1": np.ones((2, 4, 4, 3), dtype=np.uint8)})
