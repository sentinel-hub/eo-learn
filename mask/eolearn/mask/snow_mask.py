...
class SnowMask(EOTask):
    ...
    def __init__(self, data_feature, band_indices, dilation_size=0, NDSI_threshold=0.4, brightness_threshold=0.3):
        ...
        self.bands_feature_type, self.bands_feature_name = self._parse_features(data_feature).next()
    ...
    def execute(self, eopatch):
        bands = eopatch[self.bands_feature_type][self.bands_feature_name][:, :, :, self.band_indices]
        dates = bands.shape[0]
...
class TheiaSnowMask(EOTask):
    ...
    def __init__(self, data_feature, band_indices, cloud_mask_feature, DEM_feature, dilation_size=0, r_f=12, d_z=100,
                 r_b=0.1, r_d=0.3, n1=0.4, n2=0.15, r1=0.2, r2=0.040, f_s=0.001, f_t=0.1):
        """
        ...
        self.bands_feature_type, self.bands_feature_name = self._parse_features(data_feature).next()
        self.band_indices = band_indices
        self.DEM_feature_type, self.DEM_feature_name = self._parse_features(DEM_feature).next()
        self.CLM_feature_type, self.CLM_feature_name = self._parse_features(cloud_mask_feature).next()
        self.dilation_size = dilation_size
    ...
    def execute(self, eopatch):
        bands = eopatch[self.bands_feature_type][self.bands_feature_name][:, :, :, self.band_indices[:-1]]
        dates = bands.shape[0]
        B10 = eopatch[self.bands_feature_type][self.bands_feature_name][:, :, :, self.band_indices[-1]]
        DEM = eopatch[self.DEM_feature_type][self.DEM_feature_name][:, :, 0]
        clm = eopatch[self.CLM_feature_type][self.CLM_feature_name]
