...
class TestRasterToVector(unittest.TestCase):
    ...
    def test_polygon_overlap(self):
        ...
        self.assertTrue(np.sum(overlap0[overlap0 == 4]) > np.sum(overlap1[overlap1 == 4]))
        # 2 doesn't get covered, stays the same
        self.assertTrue(np.sum(overlap0[overlap0 == 2]) == np.sum(overlap1[overlap1 == 2]))
        # 10 is bbox2 and it gets covered by other classes
        self.assertTrue(np.sum(bbox1) > np.sum(overlap1[overlap1 == 10]))
        # 5 is bbox1 and it is rendered on top of all others, so it doesn't get covered
        self.assertTrue(np.sum(bbox2) == np.sum(overlap1[overlap1 == 5]))

        # all classes have their parts intersected, so the sum should reduce
        self.assertTrue(np.sum(bbox1) > np.sum(overlap2[overlap2 == 10]))
        self.assertTrue(np.sum(bbox2) > np.sum(overlap2[overlap2 == 5]))
        self.assertTrue(np.sum(overlap0[overlap0 == 4]) > np.sum(overlap2[overlap2 == 4]))
        # 2 gets covered completely
        self.assertTrue(np.sum(overlap2[overlap2 == 2]) == 0)
