...
class TDigestTask(EOTask):
    ...
    def __init__(self, in_feature, out_feature, *args, mode=None, pixelwise=True, **kwargs):
        ...
        - The 'weekly' mode computes the T-Digest representation \
            for each band accumulating the timestamps per seven days.
        - The 'daily' mode computes the T-Digest representation \
            for each band accumulating the timestamps per day.
        ...
        if isinstance(in_feature, list):
            ...
            if isinstance(out_feature, list):
                ...
                raise RuntimeError(
                    "TDigestTask: you cannot give a list of input features without specifying the a list of output"
            ...
            if isinstance(out_feature, list):
                raise RuntimeError(
                    "TDigestTask: you cannot get a list of T-Digest representations from a single feature!"
                )
            else:
                self.out_feature = [out_feature]
    ...
    def execute(self, eopatch):
            ...
            for in_feature, out_feature, pixelwise in zip(self.in_feature, self.out_feature, self.pixelwise):
                ...
                if pixelwise:
                    ...
                    if len(shape) == 4:  # add feature is temporal check
                        ...
                        raise RuntimeError(
                            f"TDigestTask: {self.mode} mode only defined for DATA, DATA_TIMELESS, MASK or MASK_TIMELESS"
                            " feature types!"
                        )
                    ...
                    if pixelwise:
                        ...
                        if len(shape) == 4:  # add feature is temporal check
                            for i, j, k in product(range(shape[1]), range(shape[2]), range(shape[3])):
                                ...
                                eopatch[out_feature][time_, i, j, k].batch_update(
                                    eopatch[in_feature][time_, i, j, k].flatten()
                                )
                        else:
                            raise RuntimeError(
                                f"TDigestTask: {self.mode} mode only defined for DATA or MASK feature types with time"
                                " component!"
                            )
        ...
        elif self.mode == "weekly" or self.mode == "daily":
