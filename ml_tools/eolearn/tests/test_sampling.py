...
@pytest.mark.parametrize('image, n_samples', [
    (np.ones((100,)), {1: 100}),
...
@pytest.mark.parametrize('n_samples, replace', [
...
def test_object_sampling_reproducibility(test_eopatch, seed):
    ...
    assert (eopatch1.mask_timeless['sampling_mask'] != eopatch3.mask_timeless['sampling_mask']).any(), \
           'Different seed produces same results'


@pytest.mark.parametrize('fraction, replace', [
    [2, False],
    [-0.5, True],
    [{1: 0.5, 3: 0.4, 5: 1.2}, False],
    [{1: 0.5, 3: -0.4, 5: 1.2}, True],
    [(1, 0.4), True]
])
def test_fraction_sampling_errors(fraction, replace):
    with pytest.raises(ValueError):
        FractionSamplingTask([(FeatureType.DATA, 'NDVI', 'NDVI_SAMPLED')], (FeatureType.MASK_TIMELESS, 'LULC'),
                             fraction=fraction, replace=replace)


@pytest.mark.parametrize('seed', range(3))
@pytest.mark.parametrize('fraction, exclude', [
    [0.2, None],
    [0.4, [0, 1]],
    [{1: 0.1, 3: 0.4}, None],
    [{2: 0.1, 3: 1, 5: 1}, []],
...
@pytest.mark.parametrize('sample_size, stride, expected_shape', [
