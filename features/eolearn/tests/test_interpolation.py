...
from eolearn.features import LinearInterpolation, CubicInterpolation, SplineInterpolation, \
    BSplineInterpolation, AkimaInterpolation, LinearResampling, CubicResampling, NearestResampling
...
class TestInterpolation(unittest.TestCase):
    ...
    class InterpolationTestCase:
        ...
        """
        TEST_PATCH_FILENAME = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                                           '../../../example_data', 'TestEOPatch')

        def __init__(self, name, task, result_len, img_min=None, img_max=None, img_mean=None,
                     img_median=None, nan_replace=None):
            self.name = name
            self.task = task
        ...
        def execute(self):
            ...
            if self.nan_replace is not None:
                ...
                patch[feature_type][feature_name][np.isnan(patch[feature_type][feature_name])] = \
                    self.nan_replace
    ...
    def setUpClass(cls):
        ...
        cls.test_cases = [
            cls.InterpolationTestCase('linear', LinearInterpolation(
                'NDVI', result_interval=(0.0, 1.0),
                mask_feature=(FeatureType.MASK, 'IS_VALID'),
                unknown_value=10),
                                      result_len=68, img_min=0.0, img_max=10.0, img_mean=0.720405,
                                      img_median=0.59765935),

            cls.InterpolationTestCase('cubic', CubicInterpolation(
                'NDVI', result_interval=(0.0, 1.0),
                mask_feature=(FeatureType.MASK, 'IS_VALID'),
            ...
            cls.InterpolationTestCase('spline', SplineInterpolation(
                'NDVI', result_interval=(-0.3, 1.0),
                mask_feature=(FeatureType.MASK, 'IS_VALID'),
            ...
            cls.InterpolationTestCase('bspline', BSplineInterpolation(
                'NDVI', unknown_value=-3,
                mask_feature=(FeatureType.MASK, 'IS_VALID'),
                resample_range=('2017-01-01', '2017-02-01', 50),
                spline_degree=5),
                                      result_len=1, img_min=-0.032482587, img_max=0.701796,
                                      img_mean=0.42080238, img_median=0.42889267),

            cls.InterpolationTestCase('akima', AkimaInterpolation(
                'NDVI', unknown_value=0,
                mask_feature=(FeatureType.MASK, 'IS_VALID')),
                                      result_len=68, img_min=-0.13793105,
                                      img_max=0.860242, img_mean=0.53159297,
                                      img_median=0.59087014),

            cls.InterpolationTestCase('nearest resample', NearestResampling(
                'NDVI', result_interval=(0.0, 1.0),
                resample_range=('2016-01-01',
                                '2018-01-01', 5)),
                                      result_len=147, img_min=-0.2, img_max=0.860242,
                                      img_mean=0.35143828, img_median=0.37481314, nan_replace=-0.2),

            cls.InterpolationTestCase('linear resample', LinearResampling(
                'NDVI', result_interval=(0.0, 1.0),
                resample_range=('2016-01-01',
                                '2018-01-01', 5)),
                                      result_len=147, img_min=-0.2, img_max=0.8480114,
                                      img_mean=0.350186, img_median=0.3393997, nan_replace=-0.2),

            cls.InterpolationTestCase('cubic resample', CubicResampling(
            ...
            cls.InterpolationTestCase('linear custom list', LinearInterpolation(
                'NDVI', result_interval=(-0.2, 1.0),
                resample_range=('2015-09-01', '2016-01-01', '2016-07-01',
                                '2017-01-01', '2017-07-01'),
                unknown_value=-2),
                                      result_len=5, img_min=-0.032482587, img_max=0.8427637,
                                      img_mean=0.5108417, img_median=0.5042224)
    ...
    def test_output_type(self):
        for test_case in self.test_cases:
            with self.subTest(msg='Test case {}'.format(test_case.name)):
                self.assertTrue(isinstance(test_case.result.timestamp, list),
                                "Expected a list of timestamps")
                ...
                self.assertEqual(ts_len, test_case.result_len,
                                 msg="Expected timestamp len {}, got {}".format(
                                     test_case.result_len, ts_len))
            ...
            if test_case.img_min is not None:
                ...
                with self.subTest(msg='Test case {}'.format(test_case.name)):
                    self.assertAlmostEqual(test_case.img_min, min_val, delta=delta,
                                           msg="Expected min {}, got {}".format(
                                               test_case.img_min, min_val))
            if test_case.img_max is not None:
                ...
                with self.subTest(msg='Test case {}'.format(test_case.name)):
                    self.assertAlmostEqual(test_case.img_max, max_val, delta=delta,
                                           msg="Expected max {}, got {}".format(
                                               test_case.img_max, max_val))
            ...
            if test_case.img_median is not None:
                ...
                with self.subTest(msg='Test case {}'.format(test_case.name)):
                    self.assertAlmostEqual(test_case.img_median, median_val, delta=delta,
                                           msg="Expected median {}, got {}".format(
                                               test_case.img_median,
                                               median_val))
